...
코드 구현/수정이 완료되면 반드시 아래 순서로 CI와 동일한 검증을 수행한다.
...
조건:
- UI 컴포넌트, 페이지, API, 테스트 파일 등 코드 변경이 수반된 모든 작업에 적용한다.
- 단순 설정 파일 변경(CLAUDE.md, .env 등)은 제외한다.
- 실패 시 원인을 수정한 후 재검증하고, 통과된 이후에 사용자에게 결과를 보고한다.
...
2. 섹션 2의 CI 전체 검증 수행 (tsc → lint → test → build)
3. E2E 대상 Task라면 `npm run test:e2e`도 추가로 통과 확인
4. 모든 검증 통과 후 사용자에게 결과 보고
