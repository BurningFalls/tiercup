...
- [ ] 내비게이션 바 (홈, 전체 탐색, + 새 티어컵 만들기)
- [ ] 티어컵 카드 컴포넌트 (썸네일, 제목, play_count, like_count, 공유/좋아요/통계 버튼)
- [ ] 페이지네이션 컴포넌트
- [ ] 검색바 컴포넌트
- [ ] 이탈 경고 모달 컴포넌트
- [ ] 티어 배지 컴포넌트 (S/A/B/C/D/F/?)

### Task 007 — 메인 페이지 구현 (F009, F010, F012)

- [ ] 인기순 / 좋아요순 / 최신순 각 4개 카드 섹션 레이아웃
- [ ] 각 섹션 [더보기] 버튼 (전체 티어컵 페이지 이동, 정렬 탭 유지)
- [ ] 검색바 (키워드 검색 → 전체 티어컵 페이지 이동)
- [ ] [+ 새 티어컵 만들기] 버튼
- [ ] 카드별 공유 버튼 / 좋아요 버튼 / 통계 버튼

### Task 008 — 전체 티어컵 페이지 구현 (F009, F010, F012)

- [ ] 정렬 탭 3개: 인기순 / 좋아요순 / 최신순 (쿼리 파라미터 `sort` 반영)
- [ ] 검색바
- [ ] 12개씩 페이지네이션
- [ ] 카드별 공유 / 좋아요 / 통계 버튼
...
- [ ] 제목 입력 (최대 30자, 글자 수 카운터)
- [ ] 아이템 입력 폼: 이미지 업로드(JPG/PNG/GIF, 5MB 이하) + 이름 입력
- [ ] 아이템 추가/삭제 버튼
- [ ] 최소 4개 미충족 시 [만들기] 버튼 비활성화
- [ ] 이탈 경고 모달 (변경사항 유실 안내)
- [ ] React Hook Form + Zod 유효성 검증 연동

### Task 010 — 플레이 화면 구현 (F002, F003, F004, F005)

- [ ] 두 아이템 카드 표시 (이미지 + 이름)
- [ ] 클릭 시 0.3초 하이라이트 애니메이션 처리
- [ ] 우측 사이드바: 전체 아이템 현재 티어 실시간 표시 (초기값 ?)
- [ ] 진행률 표시 (현재 비교 횟수 / 예상 총 비교 횟수)
- [ ] [지금 결과 확정하기] 버튼 (미비교 아이템 ? 티어 처리)
- [ ] 이탈 경고 모달 (진행 상황 유실 안내)

### Task 011 — 결과 페이지 구현 (F006, F008, F012)

- [ ] 확정된 S~F 티어 표시 (? 티어 아이템 숨김)
- [ ] 통계 표시: 비교 횟수, 총 항목 수, 소요 시간
- [ ] [티어 수정] / [다시하기] / [공유하기] 버튼
- [ ] 공유 모달: result_code 기반 링크 클립보드 복사
- [ ] 공유 링크 접속 시 읽기 전용 모드 + [나도 해보기] 버튼 고정 표시
- [ ] 좋아요 버튼

### Task 012 — 결과 수정 페이지 구현 (F007)

- [ ] @dnd-kit/core 기반 드래그앤드롭으로 아이템 개별 티어 이동
- [ ] 티어 단위 일괄 이동
- [ ] 변경사항 있을 때 [취소] 클릭 시 확인 모달
- [ ] [저장] / [취소] 버튼

### Task 013 — 통계 페이지 구현 (F011)

- [ ] 아이템별 티어 분포 히트맵 (S~F 각 티어 배치 비율)
- [ ] 재미있는 통계: S티어 1위, F티어 1위, 가장 치열한 대결
- [ ] 집계 지표: 전체 참여자 수, 총 비교 횟수, 평균 소요 시간
- [ ] [나도 해보기] 버튼

### Task 014 — 티어컵 만들기 완료 페이지 구현 (F001)

- [ ] 공유 링크 표시 및 클립보드 복사 버튼
- [ ] 관리 링크 표시 및 클립보드 복사 버튼 (재확인 불가 경고 문구)
- [ ] [지금 바로 플레이하기] 버튼
...
- [ ] BFS 위상정렬로 계층 계산
- [ ] 비율 기반 S~F 티어 매핑 로직
- [ ] Route Handler에서 실행 후 결과 반환

### Task 017 — 최적 비교 순서 결정 알고리즘 구현 (F004)

- [ ] 토너먼트 방식(n-1번) 비교 순서 생성
- [ ] 같은 계층 내 세분화 로직
- [ ] 이미 관계 있는 쌍 건너뜀 처리
